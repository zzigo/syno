...
export class AudioManager {
  ...
  private activeNodes: { osc: OscillatorNode; gain: GainNode; pan?: StereoPannerNode; chop?: AudioNode; chopInterval?: number; reverb?: AudioNode; filter?: BiquadFilterNode }[] = [];
  ...
  async play(nodes: AudioNodeType[]): Promise<void> {
    ...
    for (const node of nodes) {
      ...
      if (typeof synthNode.volume === "object") {
        ...
        gain.gain.linearRampToValueAtTime(endVol, now + startTime + duration);
        console.log(`Applying volume ramp: ${startVol} to ${endVol} over ${duration}s at ${now + startTime}`);
        ...
        gain.gain.linearRampToValueAtTime(volume, now + startTime + attack);
        gain.gain.linearRampToValueAtTime(volume * sustain, now + startTime + attack + decay);
        osc.onended = () => {
      ...
      if (synthNode.chop !== undefined) {
        chopNode = AudioProcessors.applyChop(ctx, lastNode, synthNode.chop, now + startTime);
      ...
      if (synthNode.reverb !== undefined) {
        reverbNode = AudioProcessors.applyReverb(ctx, lastNode, synthNode.reverb);
        lastNode = reverbNode;
      }
      ...
      if (synthNode.filter !== undefined) {
        ...
        if (typeof synthNode.filter === "number") {
          filterNode.frequency.value = Math.max(100, synthNode.filter * 100); // Fix: Min 100 Hz
          console.log(`Static cutoff: ${filterNode.frequency.value} Hz`);
        } else {
          const startFreq = Math.max(100, synthNode.filter.start * 100); // Fix: Min 100 Hz
          const endFreq = Math.max(100, synthNode.filter.end * 100);
          filterNode.frequency.setValueAtTime(startFreq, now + startTime);
          filterNode.frequency.linearRampToValueAtTime(endFreq, now + startTime + synthNode.filter.duration);
          console.log(`Cutoff sweep: ${startFreq} to ${endFreq} Hz over ${synthNode.filter.duration}s`);
        }
      ...
      this.activeNodes.push({ osc, gain, pan: panNode, chop: chopNode, chopInterval, reverb: reverbNode, filter: filterNode });
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    this.activeNodes.forEach(({ osc, gain, pan, chop, chopInterval, reverb, filter }) => {
      try {
        const release = gain.gain.value > 0 ? (parseInt(gain.gain.value.toString().slice(-1)) * 0.1) || 0.5 : 0.5;
        gain.gain.cancelScheduledValues(now);
        gain.gain.setValueAtTime(gain.gain.value, now);
        gain.gain.linearRampToValueAtTime(0, now + release);
  ...
  cleanup() {
    if (this.audioContext) {
      this.audioContext.close().catch(e => console.warn("Error stopping node:", e));
  ...
  getVuLevels(): { left: number; right: number } {
    if (!this.masterGain || this.activeNodes.length === 0) return { left: 0, right: 0 };
    ...
    return { left: (totalLeft / count) * masterVol, right: (totalRight / count) * masterVol };
  }

  getTimers(): number[] {
    return this.transitionManager.getActiveTimers(this.audioContext?.currentTime || 0);
  }
}