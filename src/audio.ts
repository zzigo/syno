...
export class AudioManager {
  ...
  async play(nodes: AudioNodeType[]): Promise<void> {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (
        ...
        const createdNode = this.nodeFactory.createNode(ctx, synthNode);
        if (!createdNode) continue;
        sourceNode = createdNode as OscillatorNode | AudioBufferSourceNode;
      }
      ...
      const processors = [
        {
          ...
          apply: (n: StereoPannerNode) => {
            if (typeof synthNode.pan === "object" && synthNode.pan !== null) {
              n.pan.setValueAtTime(synthNode.pan.start, now + startTime);
              n.pan.setTargetAtTime(
                synthNode.pan.end,
                now + startTime,
                synthNode.pan.duration / 4
              );
            } else if (typeof synthNode.pan === "number") {
        ...
        },
        {
          ...
          create: () =>
            AudioProcessors.applyChop(ctx, lastNode, synthNode.chop!, now + startTime),
          apply: (n: AudioNode) => {
        ...
        },
        {
          ...
          create: () => AudioProcessors.applyReverb(ctx, lastNode, synthNode.reverb!),
          apply: () => {},
        },
        {
          ...
          apply: (n: BiquadFilterNode) => {
            n.type = "lowpass";
            n.Q.value = 20;
            if (typeof synthNode.filter === "object" && synthNode.filter !== null) {
              const startFreq = Math.max(100, synthNode.filter.start * 500);
              const endFreq = Math.max(100, synthNode.filter.end * 500);
              const timeConstant = synthNode.filter.duration / 4;
              n.frequency.setValueAtTime(startFreq, now + startTime);
              n.frequency.setTargetAtTime(endFreq, now + startTime, timeConstant);
            } else if (typeof synthNode.filter === "number") {
              n.frequency.value = Math.max(100, synthNode.filter * 500);
            }
          },
        },
      ];
      ...
      // Volume
      const duration =
        typeof synthNode.volume === "object" ? synthNode.volume.duration : 20;
      ...
      if (
        ...
        sourceNode.playbackRate.setValueAtTime(startRate, now + startTime);
          ...
          const modProcessors = [
            {
              ...
              create: () => AudioProcessors.applyReverb(ctx, lastNode, modNode.reverb!),
              apply: () => {},
            },
            {
              condition: modNode.chop,
              create: () =>
                AudioProcessors.applyChop(ctx, lastNode, modNode.chop!, now + startTime),
              apply: (n: AudioNode) => {
                chopInterval = (AudioProcessors as any).chopInterval;
              },
            },
            {
              ...
              apply: (n: BiquadFilterNode) => {
                n.type = "lowpass";
                n.Q.value = 20;
                if (typeof modNode.filter === "object" && modNode.filter !== null) {
                  const startFreq = Math.max(100, modNode.filter.start * 500);
                  const endFreq = Math.max(100, modNode.filter.end * 500);
                  const timeConstant = modNode.filter.duration / 4;
                  n.frequency.setValueAtTime(startFreq, now + startTime);
                  n.frequency.setTargetAtTime(endFreq, now + startTime, timeConstant);
                } else if (typeof modNode.filter === "number") {
                  n.frequency.value = Math.max(100, modNode.filter * 500);
                }
              },
            },
          ];
          ...
          if (["s", "q", "a", "t"].includes(modNode.type)) {
            ...
            const modVolValue =
              typeof modNode.volume === "object"
                ...
                : modNode.volume ?? 5;
            const modVol = modVolValue / 9;
            modGain.gain.value = modVol * 100;
            lastNode.connect(modGain);
            const modOsc = this.nodeFactory.createNode(ctx, modNode);
            if (modOsc) {
              modOsc.connect(modGain);
              if (carrierNode instanceof OscillatorNode) {
                modGain.connect(carrierNode.frequency);
              } else if (carrierNode instanceof AudioBufferSourceNode) {
                modGain.connect(carrierNode.playbackRate);
              }
    ...
    //   if (
    //     synthNode.envelope &&
    //     "attack" in synthNode.envelope &&
    //     "decay" in synthNode.envelope &&
    //     "sustain" in synthNode.envelope &&
    //     "release" in synthNode.envelope
    //   ) {
    //     gain.gain.setValueAtTime(0, now + startTime);
    //     gain.gain.linearRampToValueAtTime(1, now + startTime + synthNode.envelope.attack);
    //     gain.gain.linearRampToValueAtTime(
    //       synthNode.envelope.sustain,
    //       now + startTime + synthNode.envelope.attack + synthNode.envelope.decay
    //     );
    //     gain.gain.setValueAtTime(
    //       synthNode.envelope.sustain,
    //       now + startTime + duration - synthNode.envelope.release
    //     );
    ...
    //   }
      ...
      if (synthNode.buffer) {
        ...
        let offlineSource: OscillatorNode | AudioBufferSourceNode | null =
          this.nodeFactory.createNode(offlineCtx, synthNode);
        if (!offlineSource) {
          throw new Error("Failed to create offline source for buffer recording");
        }
      ...
      this.activeNodes.push({
        osc: sourceNode instanceof OscillatorNode ? sourceNode : undefined,
        source:
          sourceNode instanceof AudioBufferSourceNode ? sourceNode : undefined,
        gain,
        pan: nodesChain.find((n) => n instanceof StereoPannerNode) as StereoPannerNode,
        chop: nodesChain.find((n) => n === (AudioProcessors as any).chopNode),
        chopInterval,
        reverb: nodesChain.find((n) => n === (AudioProcessors as any).reverbNode),
        filter: nodesChain.find((n) => n instanceof BiquadFilterNode) as BiquadFilterNode,
      });
    }
  }

  // Remaining methods (stop, cleanup, getVuLevels, getTimers, recordNode) unchanged
  ...
  private async recordNode(
    ...
    const offlineSource = this.nodeFactory.createNode(offlineCtx, node);
    if (!offlineSource) {
