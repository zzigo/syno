...
export class TransitionManager {
  private activeTransitions: { param: AudioParam; end: number; duration: number; startTime: number }[] = [];

  schedule(param: AudioParam, start: number, end: number, duration: number, currentTime: number) {
    param.setValueAtTime(start, currentTime);
    param.linearRampToValueAtTime(end, currentTime + duration);
    this.activeTransitions.push({ param, end, duration, startTime: currentTime });
  }
  ...
  getActiveTimers(currentTime: number): number[] {
    ...
    return this.activeTransitions.map(t => Math.floor(currentTime - t.startTime));
  }
