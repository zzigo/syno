...
A sound synthesis plugin for Obsidian, powered by the Web Audio API and written in TypeScript. Syno introduces **SYNO (Synthesis Notation)**, a concise, human-readable syntax for composing and performing generative audio directly within your Obsidian notes.
...
Here’s a comprehensive `README.md` for your Syno plugin, tailored for GitHub as an Obsidian plugin. It includes a description of SYNO (Synthesis Notation), badges for technologies used, a roadmap, and a beginner’s manual with examples for the components we’ve built so far (`sin`, `tri`, `saw`, `sqr`, `noise`, `vol`, `pan`, `chop`, `reb`, `gliss`).
...
- **`tri(x)`**: Triangle wave at `x` Hz.
  ```
  tri(300)  # Warm, mellow tone
  ```
- **`saw(x)`**: Sawtooth wave at `x` Hz.
  ```
  saw(200)  # Bright, buzzy sound
  ```
