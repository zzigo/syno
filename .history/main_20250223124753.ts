	...
	private async playSound(input: string, vuMeter: HTMLElement, playButton: HTMLElement) {
		...
		try {
			...
			for (const line of lines) {
				...
				if (!this.audioContext) {
					console.error("AudioContext is unexpectedly null again before oscillator creation");
					return;
				}
				if (!this.audioContext) {
					console.error("AudioContext is unexpectedly null AGAIN before oscillator creation");
				...
				if (!osc) {
					console.error("Oscillator node is unexpectedly null right before setting frequency");
					return;
				}
				osc.type = waveType;
				if (!osc) {
					console.error("Oscillator node is unexpectedly null again right after setting type but before frequency");
					return;
				}
				osc.frequency.value = frequency;
				if (!osc) {
					console.error("Oscillator node is unexpectedly null again right after setting frequency");
					return;
				}
					console.error("Oscillator node is unexpectedly null right before line 127");
					return;
				if (!osc) {
					console.error("Oscillator node is unexpectedly null right before line 127 - AGAIN");
					return;
				}
				if (!osc) {
					console.error("Oscillator node is REALLY unexpectedly null right before line 127 - RIGHT BEFORE FREQUENCY SET");
					return;
				}
				if (!osc) {
					console.error("Oscillator node is ABSOLUTELY NULL right before line 127 - RIGHT BEFORE FREQUENCY SET - LAST CHECK");
					return;
				}
				if (!osc) {
					console.error("Oscillator node is NULL right after creation - CHECKING IMMEDIATELY AFTER CREATEOSCILLATOR");
					return;
				}
				if (!osc) {
					console.error("Oscillator node is STILL NULL right after creation - DOUBLE CHECK IMMEDIATELY AFTER CREATEOSCILLATOR");
					return;
				}
				...
				if (!gain) {
					console.error("Gain node is unexpectedly null right before setting gain value");
					return;
				}
				...
				if (!osc) {
					console.error("Oscillator node is unexpectedly null right before connecting to gain node");
					return;
				}
