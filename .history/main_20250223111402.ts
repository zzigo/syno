...
export default class SynoPlugin extends Plugin {
	...
	private async processSynoBlock(source: string, el: HTMLElement) {
		...
		// Create Container for Play Button + VU Meter (Bottom-Right Aligned)
		...
		// Create ASCII VU Meter (Single Character)
		const vuMeter = controlsContainer.createEl("span", { text: "_" }); // Initial meter
	...
	private async playSound(vuMeter: HTMLElement) {
					...
					process(inputs, outputs, parameters) {
						...
						for (let channel = 0; channel < output.length; channel++) {
							...
							for (let i = 0; i < channelData.length; i++) {
								...
								channelData[i] = Math.sin(this.phase);
		...
		this.audioNode.port.onmessage = (event) => {
			...
			vuMeter.textContent = "≡".repeat(Math.max(1, level)); // Ensure at least 1 character
		};
	}
