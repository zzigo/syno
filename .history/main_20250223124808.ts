	...
	private async playSound(input: string, vuMeter: HTMLElement, playButton: HTMLElement) {
		...
		try {
			...
			for (const line of lines) {
				...
				// Connect Nodes
				osc.connect(gain);
				panNode.connect(this.masterGain!); // masterGain is ensured to be not null in `playSound`
			...
			const interval = setInterval(() => {
				...
				for (const node of this.activeNodes) {
					if (node.gain && node.gain.gain) { // Check if node.gain and node.gain.gain are not null
