...
export default class SynoPlugin extends Plugin {
	...
	private async playSound(input: string, vuMeter: HTMLElement, playButton: HTMLElement) {
		...
		this.activeNodes = []; // Clear activeNodes at the beginning
		this.cleanupAudio(); // Clean up any existing audio context
		this.audioContext = new AudioContext(); // Create a new audio context for each play
		try {
			...
			// Reset audio context - already done above
			// this.cleanupAudio();
			...
			for (const line of lines) {
				...
				osc.type = waveType;
				if (!osc) {
					console.error("Oscillator is unexpectedly null before setting frequency");
					return;
				}
				if (!osc) {
					console.error("Oscillator node is unexpectedly null AGAIN right before setting frequency");
					return;
				}
				...
				osc.connect(gain);
				if (this.masterGain) { // Check if masterGain is not null
					gain.connect(panNode).connect(this.masterGain); // Connect panNode to masterGain only if masterGain is not null
				} else {
					console.error("masterGain is null, cannot connect panNode");
					return;
				}
			...
			const interval = setInterval(() => {
				...
				for (const node of this.activeNodes) {
					if (node.gain?.gain) { // Use optional chaining to avoid errors if node.gain is null
						avgVolume += node.gain.gain.value;
					}
				}
				avgVolume /= this.activeNodes.length;
