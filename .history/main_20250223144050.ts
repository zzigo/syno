...
export default class SynoPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.registerMarkdownPostProcessor((el, ctx) => {
            this.processInlineSyno(el, ctx);
        });

        // Ensure Live Preview updates on significant events
        this.registerEvent(
            this.app.workspace.on("active-leaf-change", () => {
                const view = this.app.workspace.getActiveViewOfType(require("obsidian").MarkdownView);
                if (view) {
                    this.processInlineSyno(view.contentEl, { docId: view.file?.path || "" } as any);
                }
            })
        );
    }
    ...
    private processSynoBlock(source: string, el: HTMLElement, ctx: MarkdownPostProcessorContext) {
        ...
        // Render with Python highlighting
        const codeBlock = container.createEl("pre", { cls: "language-python" });
        const code = container.createEl("code", { cls: "language-python", text: source });
        codeBlock.appendChild(code);
        // Use Obsidian's built-in highlighting
        (this.app as any).highlightCodeBlock(code, "python", source);
    ...
    /** Process Inline Syno */
    private processInlineSyno(el: HTMLElement, ctx: MarkdownPostProcessorContext) {
        ...
        while ((node = walker.nextNode())) {
            ...
            if (text.includes("&") && text.includes("&")) {
                ...
                updatedContent = updatedContent.replace(printRegex, (_match, message) => {
                ...
                updatedContent = updatedContent.replace(synthRegex, (_match, synthCode) => {
                    ...
                    return `<span class="syno-inline-synth language-python">${synthCode}</span><span class="syno-vumeter">_</span><span class="syno-play-button" style="cursor:pointer;">▷</span>`;
                });

                node.parentNode?.replaceChild(document.createRange().createContextualFragment(updatedContent), node);
                walker.nextNode(); // Skip the newly added nodes
            }
        }

        // Apply highlighting and play button handlers
        el.querySelectorAll(".syno-inline-synth").forEach((elem) => {
            const code = elem as HTMLElement;
            (this.app as any).highlightCodeBlock(code, "python", code.textContent || "");
        });
        ...
        el.querySelectorAll(".syno-play-button").forEach((btn) => {
            ...
            const vuMeter = btn.previousElementSibling as HTMLElement;
            if (!synthSpan || !vuMeter || !synthSpan.classList.contains("syno-inline-synth")) return;

            const synthCode = synthSpan.textContent || "";
            let isPlaying = false;

            btn.removeEventListener("click", btn.onclick as any);
            btn.onclick = async () => {
                if (isPlaying) {
                    await this.stopSound(vuMeter, btn as HTMLElement);
                    isPlaying = false;
                } else {
                    await this.playSound(synthCode.trim(), vuMeter, btn as HTMLElement);
                    isPlaying = true;
                }
            };
        });
    ...
    private parseRampingParam(input: string, type: "vol" | "pan" | "freq"): number | { start: number; end: number; duration: number } {
        ...
        if (end === undefined || duration === undefined) {
            if (type === "vol" && (start < 0 || start > 1)) {
                console.error(`Invalid ${type} value:`, start);
                return 0.5;
            }
            if (type === "pan" && (start < -1 || start > 1)) {
                console.error(`Invalid ${type} value:`, start);
                return 0;
            }
            if (type === "freq" && start <= 0) {
                console.error(`Invalid ${type} value:`, start);
                return 440;
            }
            return start;
