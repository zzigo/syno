...
export default class SynoPlugin extends Plugin {
    ...
    async onload() {
        ...
        // Register post-processor for Reading Mode and Live Preview
    ...
    /** Process Syno Block */
    private processSynoBlock(source: string, el: HTMLElement, ctx: any) {
        ...
        // Use Obsidian's CodeMirror for highlighting
        const cm = (this.app as any).plugins.plugins["obsidian"]?.CodeMirror;
        if (cm && cm.runMode) {
            cm.runMode(source, "python", code);
        } else {
            console.warn("CodeMirror not available, falling back to plain text");
        }
    ...
    /** Process Inline Syno */
    private processInlineSyno(el: HTMLElement, ctx: any) {
        const textContent = el.innerHTML;
        const decodedContent = textContent.replace(/&/g, "&").replace(/</g, "<").replace(/>/g, ">");
        ...
        updatedContent = updatedContent.replace(synthRegex, (_match, synthCode) => {
            ...
            const synthSpan = `<span class="syno-inline-synth language-python">${synthCode}</span>`;
        ...
        // Apply Python highlighting to inline synths
        const cm = (this.app as any).plugins.plugins["obsidian"]?.CodeMirror;
        el.querySelectorAll(".syno-inline-synth").forEach((elem) => {
            const code = elem as HTMLElement;
            if (cm && cm.runMode) {
                cm.runMode(code.textContent || "", "python", code);
            } else {
                console.warn("CodeMirror not available for inline highlighting");
            }
        });
