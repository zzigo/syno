...
import { around } from "monkey-around"; // For patching Live Preview
...
export default class SynoPlugin extends Plugin {
    ...
    async onload() {
        ...
        // Patch Live Preview rendering
        this.registerLivePreviewPatch();
    }
    ...
    /** Register Live Preview Patch */
    private registerLivePreviewPatch() {
        const app = this.app;
        this.register(around(app.workspace, {
            onLayoutReady(old) {
                return function () {
                    old.call(this);
                    const cmEditors = this.getLeavesOfType("markdown").map(leaf => (leaf.view as any).sourceMode?.cmEditor);
                    cmEditors.forEach((cm: any) => {
                        if (cm && cm.getValue) {
                            cm.on("change", () => {
                                const view = cm.state.field(app.workspace.activeLeaf.view.editorInfoField);
                                if (view) {
                                    const contentEl = view.contentEl;
                                    this.processInlineSyno(contentEl);
                                }
                            });
                        }
                    });
    ...
    private processSynoBlock(source: string, el: HTMLElement) {
        ...
        this.app.marked.highlightElement(code); // Use Obsidian's built-in highlighter
    ...
    private processInlineSyno(el: HTMLElement) {
        ...
        updatedContent = updatedContent.replace(synthRegex, (_match, synthCode) => {
            ...
            // Use Python highlighting for inline synths
        ...
        // Apply highlighting to inline synths
        el.querySelectorAll(".syno-inline-synth").forEach((elem) => {
            this.app.marked.highlightElement(elem as HTMLElement);
        });
    ...
    private createAndStartNode(parsed: ReturnType<typeof this.parseInput>, blockVolume?: any, blockPan?: any, blockEq?: number[] | null) {
        ...
        if (typeof effectiveVolume === "number") {
            ...
            if (effectiveVolume.end !== undefined && effectiveVolume.duration) {
                ...
                gain.gain.linearRampToValueAtTime(effectiveVolume.end, this.audioContext.currentTime + effectiveFreq.duration);
            } else {
